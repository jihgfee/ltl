...
Section ltl_proofmode.
  ...
  Proof. 
    iSplit.
    - iIntros "#[HP HQ]". iSplit; by iIntros "!>".
    - iIntros "[#HP #HQ]". eauto. 
  Qed.
  ...
  Proof.
    rewrite /IntoWand.
    destruct p; simpl.
    - intros HR. iIntros "#HR #HP". iNext. by iApply HR.
  ...
  Lemma test (P Q R : ltl_pred) :
    (□ P ∧ Q ⊢ ◊ Q) → (□ P ∧ ◊ Q ⊢ ◊ R).
  Proof. Admitted.

  Tactic Notation "iEv" :=
    iApply (test with "[-]"); last first; [iSplitR;try eauto|].

  Lemma test' (P1 P2 Q R : ltl_pred) :
    (□ P1 ∧ □ P2 ∧ ◊ Q ⊢ ◊ R).
  Proof.
    iIntros "[#HP1 [#HP2 HR]]".
    iEv.
    { iFrame "HR". iFrame "".
    iApply (test with "[HP $HR]"); last first.
    { iExact "HP". }
    iIntros "[#HP HQ]".
    iModIntro. done.
  Qed.

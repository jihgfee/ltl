...
Section ltl_lemmas.
 ...
 (*  Proof. done. Qed. *)
  ...
  Lemma trace_always_intro' (P : ltl_pred) :
    (⊢ P) → (⊢ □ P)%trace.
  Proof.
    intros HP tr _. apply trace_alwaysI.
  ...
  Lemma trace_and_intro (P Q R : ltl_pred) :
    (P ⊢ Q) → (P ⊢ R) → (P ⊢ Q ∧ R).
  Proof.
    intros HPQ HPR. intros tr. rewrite trace_andI.
  ...
  Proof.
    ...
    - intros tr. rewrite trace_andI. intros [HP HQ] HPQ. induction HPQ.
      { apply H. apply trace_andI. apply trace_always_elim in HP, HQ. done. }
      by apply IHHPQ; eapply trace_always_cons.
  Qed.
...
Definition trace_wand {S L} : ltl_pred S L → ltl_pred S L → ltl_pred S L :=
  trace_implies.
Definition trace_persistently {S L} (P : ltl_pred S L) : ltl_pred S L :=
  P.
Definition trace_plainly {S L} (P : ltl_pred S L) : ltl_pred S L := trace_always P.
...
Lemma ltl_bi_mixin {S L} :
  BiMixin
    ...
    (@trace_forall S L) (@trace_exists S L) trace_sep trace_wand.
Proof. Admitted.
(*   split. *)
...
Lemma ltl_bi_persistently_mixin {S L} :
  BiPersistentlyMixin
    ...
    (@trace_exists S L) trace_sep trace_persistently.
Proof. Admitted.
(*   split. *)
(*   - admit. *)
(*   - intros P Q HPQ.  *)
...
Definition trace_later {S L} (P : ltl_pred S L) : ltl_pred S L := P.

Lemma ltl_bi_later_mixin {S L }:
  BiLaterMixin
    trace_entails trace_pure trace_or trace_implies
    (@trace_forall S L) (@trace_exists S L) trace_sep trace_persistently trace_later.
Proof. Admitted.
  (* split. *)
...
(*   sealing lemmas are partially applied so that they also work under binders. *)
  ...
  Proof. Admitted.
  Local Lemma ltl_later_unseal : bi_later = @trace_later S L.
  Proof. Admitted.
...
Section ltl_proofmode.
  ...
  Lemma test (P Q : ltl_pred) : ⊢ P → (P → Q) → Q.
  Proof.
    iIntros "HP HPQ". iApply "HPQ". done.
  Qed.
  ...
  Lemma bar_I {PROP : bi} (P Q : PROP) :
    (P ⊢ Q)%I → (P ⊢ Q)%I.
  Proof.
    iIntros (HPQ) "HP".
    iDestruct HPQ as "HPQ".
    iDestruct ("HPQ" with "HP") as "HQ".
    done.
  Qed.

  Lemma bar_trace (P Q : ltl_pred) :
    (P ⊢ Q)%I → (P ⊢ Q)%I.
  Proof.
    iIntros (HPQ) "HP".
    
    iDestruct HPQ as "HPQ".
    (* iDestruct ("HPQ" with "HP") as "HQR". *)
  Admitted.
  ...
  Proof.
    ...
    eapply (trace_always_mono' _ (◊ (trace_and P Q))).
    { apply trace_eventually_mono'. by iIntros "[_ HQ]". }
    apply trace_always_intro.
    { by rewrite {1}HPQ -trace_eventually_next_comm' trace_eventually_next'. }
    rewrite -trace_eventually_next_comm'.
    iIntros "HPQ".
    iEval (rewrite -trace_eventually_idemp').
    iStopProof. apply trace_eventually_mono'.
    iIntros "[HP _]". by rewrite {1}HPQ trace_eventually_next_comm'.
  Qed.

...
Section ltl_lemmas.
  ...
  Global Instance ltl_entails_po : PreOrder (@trace_entails S L).
  Proof.
    split.
    - intros tr. apply (trace_entails_refl).
    - intros tr. apply (trace_entails_trans).
  Qed.
...
Section ltl_meta.
  ...
  Lemma ltl_entails_anti_symm : AntiSymm (≡) (@trace_entails S L).
...
Section ltl_theorems.
  ...
  (* Lemma SF_implies_OF (P Q: ltl_pred) : *)
  (*   ⊢ (((□◊ P) → (□◊ Q)) → (□ ((□◊ P) → (◊ Q)))). *)
  (* Proof. *)
  (*   iApply trace_always_intro. *)
  (*   { iIntros "HPQ HP". by iMod ("HPQ" with "HP") as "HQ". } *)
  (*   iIntros "HPQ". *)
  (*   { . *)

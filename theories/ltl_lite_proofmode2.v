...
Lemma ltl_bi_later_mixin {S L }:
  BiLaterMixin
    ...
    (@trace_forall S L) (@trace_exists S L) trace_and trace_persistently trace_later.
Proof. Admitted.
  (* split. *)
...
Section ltl_proofmode.
  ...
  Global Instance into_wand_next p q (P Q R : ltl_pred) :
    IntoWand p q R P Q → IntoWand p q (○ R)%trace (○ P)%trace (○ Q)%trace.
  Proof. Admitted.
  Global Instance into_wand_next_args p q (P Q R : ltl_pred) :
    IntoWand p q R P Q → IntoWand' p q (R)%trace (○ P)%trace (○ Q)%trace.
  Proof. Admitted.

  Global Instance into_wand_always p q (P Q R : ltl_pred) :
    IntoWand p q R P Q → IntoWand p q (□ R)%trace (□ P)%trace (□ Q)%trace.
  Proof. Admitted.
  Global Instance into_wand_always_args p q (P Q R : ltl_pred) :
    IntoWand p q R P Q → IntoWand' p q (R)%trace (□ P)%trace (□ Q)%trace.
  Proof. Admitted.
  ...
  Proof.
    destruct b; simpl.
    - rewrite /IntoAnd. simpl. intros HPQ.
      iIntros "#HP". 
      iDestruct (HPQ with "HP") as "HQ".
      iModIntro.
      iEval (rewrite -(trace_always_and'' Q1 Q2)).
  Admitted.
  ...
  (* Class EventuallyForall (P Q : ltl_pred) : Prop. *)
  (* Instance eventually_transform_eventually (P : ltl_pred) : *)
  (*   EventuallyTransform (◊ P) P. *)
  (* Proof. Qed. *)
  (* Lemma modality_eventually_mixin : modality_mixin (@id ltlI) *)
  (*   (MIEnvTransform EventuallyTransform) (MIEnvTransform EventuallyTransform). *)
  (* Proof. split; simpl; eauto. Admitted. *)
  ...
  Proof.
    ...
    { iIntros "HP". by iMod (HPQ with "HP") as "HPQ". }
    iIntros "HPQ". iMod "HPQ" as "[HP _]". by iMod (HPQ with "HP").  Qed.
